...
// ... (Il resto del codice per handleStatusCommand, startServerStatusLoop e createStatusEmbed rimane ASSOLUTAMENTE IDENTICO a quello che abbiamo già testato e che funziona)

// ⚠️ IMPORTANTE: Sostituisci questa sezione finale del codice
  ...
  setInterval(async () => {
    try {
      ...
      const hasMeaningfulChange = 
        ...
        (now - lastUpdateTime) > config.cooldownPeriod * 5;
...
function createStatusEmbed(data, isAutoUpdate) {
  ...
  if (isAutoUpdate) {
    embed.setFooter({ text: '🔄 Aggiornamento automatico - Cooldown 150s' });
  }
...
// Avvia il bot usando il token da config.json
